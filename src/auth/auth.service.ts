...
export class AuthService {
  ...
  /**
   * Registro de nuevo usuario con validaciones de unicidad, hashing,
   * verificación de correo, seteo de cache, y manejo seguro de errores.
   */
  async createUser(dto: CreateUserDto, res?: Response) {
    const email = dto.email.toLowerCase().trim();

    try {
      // Revisar en caché si ya existe el correo (por seguridad y rendimiento)
      const cached = await this.cacheManager.get(`register_email_block_${email}`);
      if (cached) {
        throw new ConflictException('Este correo ya está siendo procesado. Intenta más tarde.');
      }
      // Bloquear temporalmente para evitar múltiples intentos simultáneos
      await this.cacheManager.set(`register_email_block_${email}`, true, 30);

      // Validar si el correo ya está registrado
      const exists = await this.prisma.users.findUnique({ where: { email } });
      if (exists) throw new ConflictException('El correo ya está registrado');

      // Hashear la contraseña con salt fuerte
      ...
      // Setear cookie opcional si se provee res (ej: token temporal)
      if (res) {
      ...
      // Borrar bloqueo en caché en caso de error
      await this.cacheManager.del(`register_email_block_${email}`);
