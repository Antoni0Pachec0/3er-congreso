...
  // ======= Implementación de métodos auxiliares (safeRetry) omitida para brevedad =======
...
    // CAMBIO CRUCIAL: Buscar cualquier token válido (email_verification O reset_password)
    const token = await this.prisma.verification_token.findFirst({
      where: {
        user_id: user.user_id,
        token: code,
        // Buscamos ambos tipos de token
        token_type: { in: ['email_verification', 'reset_password'] }, 
        used: false,
...
    if (!token) {
      // Solo incrementamos si era un token de activación (ya que forgot no tiene intentos)
      if (await this.shouldIncrementAttempt(user.user_id)) {
...
    // 1) Activar usuario SOLO si es token de verificación de cuenta (registro)
...
      where: {
        user_id: user.user_id,
        token_type: token.token_type, // Invalidar solo tokens del mismo tipo
        used: false,
...
    // DEVOLVER el tipo de token para la redirección en el frontend
    return { 
      message: 'Código verificado exitosamente', 
      user_id: Number(user.user_id),
      token_type: token.token_type // <-- NUEVO CAMPO
    };
  }

  // Función auxiliar para manejar el incremento de intentos de forma más segura
  private async shouldIncrementAttempt(userId: bigint): Promise<boolean> {
    // Buscamos si existe un token de activación reciente para evitar incrementar intentos
    // en flujos de reset password donde no se usa este mecanismo.
...
    // ... (resto de la lógica de resendCode permanece igual, solo maneja 'email_verification')
...
  // ======= El resto de los métodos (refreshToken, getProfile, logout) permanecen sin cambios. =======
